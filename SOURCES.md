...
This Git repository contains authored source, the npm lockfile, native build recipes, the guest CLI patch and third-party license notices. Generated bundles, executables, upstream archives, model weights, firmware and recorded sessions are not committed.

| Component | Pin / source | License / handling |
|---|---|---|
| Tutorial UI, simulator, guest bridge and picnic MCP | This repository | MIT (`LICENSE`) |
| term-llm guest CLI | [Upstream08059d2dceb6606e8f190ab3e88103dc53d03b66](https://github.com/samsaffron/term-llm/tree/08059d2dceb6606e8f190ab3e88103dc53d03b66), plus `sources/term-llm-artifact.patch` | MIT; dependency notices in `licenses/go/` |
...
| xterm / FitAddon | npm6.0.0 /0.11.0; lockfile authoritative | MIT |
...
`sources/runtime-manifest.json` records the exact byte sizes, SHA-256 hashes and public URLs for already-tested native runtime files, v86 assets, shell bundle and required source archives. `scripts/fetch-runtime.mjs` verifies every download before installing it under ignored directories. It never executes downloaded programs. Native CLI rebuilding instructions are in the README; the patch includes the additional browser-workspace files as well as tracked modifications.
