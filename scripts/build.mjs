...
const wterm=execFileSync('bash',['scripts/build-wterm.sh'],{encoding:'utf8',stdio:['ignore','pipe','inherit']}).trim();
await build({alias:Object.fromEntries(['dom','core','ghostty'].map(p=>['@wterm/'+p,wterm+'/packages/@wterm/'+p+'/dist/index.js'])),entryPoints:['public/app.mjs'],outfile:'public/app.js',bundle:true,format:'esm',external:['./assets/libv86.mjs'],minify:true,define:{__GUEST_WEB_SW_HASH__:JSON.stringify(createHash('sha256').update(await readFile('public/guest-web/sw.js')).digest('hex').slice(0,20))}});
...
await copyFile(wterm+'/packages/@wterm/ghostty/wasm/ghostty-vt.wasm','public/assets/ghostty-vt.wasm');
