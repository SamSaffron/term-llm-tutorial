...
for(const name of ['index.html','app.css','app.js','inference-worker.js','bonsai-worker.js','gemma-worker.js','simulator-worker.js','boot.sh','guest-config.yaml','assets/term-llm','assets/guest-bridge','assets/git','assets/picnic-mcp','assets/zsh-root.tar.gz','zsh-source.tar.gz','assets/v86.wasm','assets/libv86.mjs','assets/ghostty-vt.wasm','assets/wterm.css'])await copyFile(`public/${name}`,`${root}/${name}`);
