...
test('image generation serializes, preserves metadata, unloads on failure and can reload',async()=>{
 ...
 const job=images.generate({prompt:'a pelican',seed:1});assert.equal(images.state,'generating');await assert.rejects(images.generate({prompt:'another'}),/Janus is not loaded/);
...
test('license consent and separate canned command are present; pinned runtime is lazy',async()=>{
 const html=await readFile('public/index.html','utf8');assert.match(html,/§5 and all Attachment A/);assert.match(html,/href="\/" class="wordmark"/);assert.match(html,/Demo returns canned drawings/);
